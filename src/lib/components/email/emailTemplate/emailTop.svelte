...
<div>
  {#if $page.url.pathname !== "/"}
    {#if $metaStore.screen === "desktop"}
      <div class="py-4 px-6">
        <div class="flex justify-between items-center ishtyle pb-3 mb-2">
          <div class="flex items-center">
            <h2 class="bg-background text-3xl font-semibold w-fit px-3 py-1">
              {pageName}
            </h2>
            <div
              class="ml-4 text-xs text-slate-200 bg-slate-900 px-2 py-1 rounded-md"
            >
              Inbox
            </div>
          </div>
